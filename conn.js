...
const { jadibot, listJadibot, stopjadibot} = require('./function/jadibot')
...
const isOwner = [`${setting.ownerNumber}`,"62851567568604@s.whatsapp.net","62851567568605@s.whatsapp.net"].includes(sender) ? true : false
...
"fileName": `Memek-MD ${ngazap(prefix)}`,
...
let ii = await fetchJson(`https://danzzapi.xyz/api/shortlink/bitly?url=${data_deposit.data.trannss}&apikey=92y1a7l0a6`)
...
if (chats.length === 0) return;
data_deposit.data.trannss = (chats)
let ii = await fetchJson(`https://danzzapi.xyz/api/shortlink/tinyurl?url=${data_deposit.data.trannss}&apikey=92y1a7l0a6`)
if (ii.status == false) return reply('url tidak valid\nsilahkan kirim yg benar.')
data_deposit.session = "input_texttttranss";
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_deposit, null, 3));
reply(`*SHORTLINK*
...
if (chats.length === 0) return;
data_deposit.data.trannss = (chats)
let ii = await fetchJson(`https://danzzapi.xyz/api/shortlink/cuttly?url=${data_deposit.data.trannss}&apikey=92y1a7l0a6`)
if (ii.status == false) return reply('url tidak valid\nsilahkan kirim yg benar.')
data_deposit.session = "input_texttttranss";
fs.writeFileSync(PathAuto + sender.split("@")[0] + ".json", JSON.stringify(data_deposit, null, 3));
reply(`*SHORTLINK*
...
_Admin 1 : 0851-5675-6860_
_Admin 2 : 0851-5675-68604_
...
const gurbot = '6285156756860@s.whatsapp.net'
const mark_slebew = '62851567568605@s.whatsapp.net'
...
let btn_menu = [
{buttonId: `${prefix}groupbot`, buttonText: { displayText: '⋮☰ *TOP UP GAME*' }, type: 1 },
...
conn.sendMessage(from, { caption: cap, image: { url: 'https://member.net-pedia.site/assets/images/produk/qr_qris.png' }, buttons: buto, footer: '© created by net digital' })
}
console.log(feta)
}
break
...
var dev = '6285156756860@s.whatsapp.net'
